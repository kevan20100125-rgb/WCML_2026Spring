...
SNR_train = [0, 5, 10, 15, 20, 25]  # SNR list
test = True  # test or trainP
BER = []
...
class SysIn(object):
    ...
    ## Seleted the detection type 
    detect_type = 'EPNet'  # 'EP', 'EPNet' 'CG_OAMP_NET', 'OAMP_NET', 'CG_OAMP', 'OAMP', 'MAMP', 'ZF' or 'MMSE'
    ##
...
class TrainSetting(object):
    CG = False  # CG-OAMP-NET or OAMP-NET
    net = 'EPNet'
    T, icg = 5, 50  # number of iterations
    ...
    savefile = 'EPNet_QPSK_8_8_20dB_T4.npz'
...
# train
if test is False:
    if trainSet.net == 'EPNet':
        sys.sess, sys.x_hat_net = networks.build_EP(trainSet)
    elif trainSet.CG:
        """ CG-OAMP-NET """
...
# test
    ...
    if sys.detect_type == 'EPNet':  # load the trained model
        sys.sess, sys.x_hat_net = networks.build_EP(trainSet)
    elif sys.detect_type == 'CG_OAMP_NET':  # load the trained model
        sys.sess, sys.x_hat_net = networks.build_CG_OAMP(trainSet)
    elif sys.detect_type == 'OAMP_NET':  # load the trained model
        sys.sess, sys.x_hat_net = networks.build_OAMP(trainSet)
