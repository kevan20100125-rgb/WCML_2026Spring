...
def train_amp_nn(test=False, trainset=None):
    ...
    def grad(x, y, H, sigma2, bs, label):
        with tf.GradientTape() as tape:
            xhat, loss, mse = model(x, y, H, sigma2, bs=bs, test=test, label=label)
    ...
    for i in range(maxit + 1):
        if i % ivl == 0:  # validation:don't use optimizer
            ...
            for m in range(total_batch):
                ...
                xhat, loss_batch, mse = model(xbatch, ybatch, Hbatch, sigma2batch,
        ...
        for m in range(total_batch):
            ...
            loss_batch, grads = grad(xbatch, ybatch, Hbatch, sigma2batch, batch_size,
                                     label[m * batch_size:(m + 1) * batch_size])
...
class AMP_NN(Model):
    ...
    def __call__(self, x, y, A, noise_var, label=tf.zeros([1], tf.float64), bs=None, test=False):
        ...
        for t in range(self.iter):
            w = tf.matmul(A, ave) - tf.matmul(sqrA, var) * (y - w) / (noise_var + v)  # note: nominator vt, denominator vt-1
