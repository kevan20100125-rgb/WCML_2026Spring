...
try:
    for k,v in np.load("model/64QAM_8×6_20dB_SUI_T4_.npz").items():
        para[k] = v
except IOError:
    print("no such file")
    pass
...
def CG_OAMP(x,A,y,noise_var,T=5,I=50,mu=2):
    ...
    for t in range(T):
        ...
        sigmoid_beta = 1. / (1. + np.exp(-beta[t]))
        v_sqr = sigmoid_beta * v_sqr + (1 - sigmoid_beta) * v_sqr_last
