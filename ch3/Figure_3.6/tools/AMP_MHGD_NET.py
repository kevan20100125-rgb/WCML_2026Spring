...
def train_amp_mhgd_net(test=False, trainset = None):
    ...
    x_amp, loss_, mse = model1.build(x_, y_, H_, sigma2_, bs=bs_, test=test)  # transfer place holder and build the model
    ...
    xhat, loss_, mse = model2.build(x_, y_, H_, sigma2_,
                                    bs=bs_, test=test, x_fs=x_amp)  # transfer place holder and build the model
