...
def train_mhgd_net(test=False, trainset = None):
    ...
    model = MHGDNet(trainset=trainset)
...
class MHGDNet:
    def __init__(self, trainset=None):
        ...
        self.d_tune, self.vec_step_size, self.r_tune = trainset.d_tune, trainset.vec_step_size, trainset.r_tune
        self.nolr = trainset.nolr
        self.n_scale = trainset.n_scale
        ...
        for t in range(self.iter):
            ...
            if self.r_tune:
                self.gamma.append(tf.Variable(1.0, dtype=tf.float64, name='gamma_' + str(t), trainable=True))
            if self.n_scale:
                self.sigma.append(tf.Variable(1.0, dtype=tf.float64, name='sigma_' + str(t), trainable=True))
    ...
    def build(self, x, y, A, noise_var, bs=None, test=False, x_fs=None):
        ...
        if self.lr_approx is False and self.nolr is True:
            p_mat = tf.matmul(tf.matmul(A, grad_preconditioner), AH)
        else:
            p_mat = None
        ...
        if self.mmse_init is True:
        ...
        elif self.mmse_init == 'fs' or self.mmse_init == 'amp':
            # xhat = x_fs  # (bs, nt, 1)
            # quantization
            ...
            xhat = tf.gather_nd(self.constellation_norm, indices[:, :, tf.newaxis])[:, :, tf.newaxis]  # (bs, nt, 1)
            if test:
                # x_fs = tf.reshape(tf.tile(x_fs, [self.samplers, 1, 1]), [self.samplers, bs, self.nt, 1])
                # indices = tf.random.uniform(shape=[self.samplers, bs, self.nt, 1], maxval=2 ** self.mu, dtype=tf.int64)
                # xhat = tf.gather_nd(self.constellation_norm, indices)[:, :, :, tf.newaxis]  # (np, bs, nt, 1)
            ...
            xhat = tf.gather_nd(self.constellation_norm, indices[:, :, tf.newaxis])[:, :, tf.newaxis]  # (bs, nt, 1)
            if test:
                # x_cg = tf.reshape(tf.tile(xhat, [self.samplers, 1, 1]), [self.samplers, bs, self.nt, 1])
                # indices = tf.random.uniform(shape=[self.samplers, bs, self.nt, 1], maxval=2 ** self.mu, dtype=tf.int64)
                # xhat = tf.gather_nd(self.constellation_norm, indices)[:, :, :, tf.newaxis]  # (np, bs, nt, 1)
                # mask = np.zeros(self.samplers, dtype=bool)
                # mask[0] = True
                # mask = tf.reshape(tf.convert_to_tensor(mask), [self.samplers, 1, 1, 1])
                # mask = tf.tile(mask, [1, bs, self.nt, 1])
                # xhat = tf.where(condition=mask, x=x_cg, y=xhat)  # only one cg
                xhat = tf.reshape(tf.tile(xhat, [self.samplers, 1, 1]), [self.samplers, bs, self.nt, 1])  # all cg
        else:
            if test:
                indices = tf.random.uniform(shape=[self.samplers, bs, self.nt, 1], maxval=2 ** self.mu, dtype=tf.int64)
                xhat = tf.gather_nd(self.constellation_norm, indices)[:, :, :, tf.newaxis]  # (np, bs, nt, 1)
        ...
        if self.lr_approx is False and self.nolr is True:
            pr_prev = tf.matmul(p_mat, r)  # (bs, nr, 1) or (np, bs, nr, 1)
            if test:  # (np, bs, 1, 1)
                pr_prev_norm = tf.cast(tf.norm(pr_prev, axis=2, keepdims=True) ** 2, dtype=tf.float64)
            else:  # (bs, 1, 1)
                pr_prev_norm = tf.cast(tf.norm(pr_prev, axis=1, keepdims=True) ** 2, dtype=tf.float64)
            self.lr[0] = tf.math.real(tf.matmul(tf.linalg.adjoint(r), pr_prev)) / pr_prev_norm

        dqam_for_step = self.dqam
        if self.d_tune:
            # dqam_for_step *= tf.minimum(self.gamma[0], self.es)  # dqam for step less than one to avoid too large jump
            dqam_for_step *= (self.es * tf.math.sigmoid(self.gamma[0]))  # dqam for step less than one to avoid too large jump
        if self.vec_step_size:
            ...
            if self.r_tune:
                step_size = tf.maximum(dqam_for_step,
                                       tf.math.sqrt(r_norm) / self.nt * tf.maximum(self.gamma[0], 0)) * alpha  # (bs, 1, 1) float64
            else:
                step_size = tf.maximum(dqam_for_step,
        ...
        for t in range(self.iter):
            ...
            z_grad = xhat + tf.matmul(tf.matmul(tf.cast(tf.minimum(self.lr[t], 5), dtype=tf.complex128)
                                                * grad_preconditioner, AH), r)  # constrain lr to be too large
            ...
            # acceptance step
            if self.n_scale:
                sigma = tf.maximum(self.sigma[t], tf.cast(noise_var, dtype=tf.float64))
                log_pacc = tf.minimum(tf.cast(0., dtype=tf.float64),
                                      -(r_norm_prop - r_norm) / (tf.cast((2), dtype=tf.float64) * sigma))  # float64
            else:
                log_pacc = tf.minimum(tf.cast(0., dtype=tf.float64),
            ...
            if test:
                ...
                if self.n_scale:
                    p_uni = tf.constant(0.5, dtype=tf.float64)
            ...
            # learning rate calculation
            if self.lr_approx is False and self.nolr is True:
                pr_prev = p_mat @ r  # (bs, nr, 1) or (np, bs, nr, 1)
                if test:  # (np, bs, 1, 1)
                    pr_prev_norm = tf.cast(tf.norm(pr_prev, axis=2, keepdims=True) ** 2, dtype=tf.float64)
                else:  # (bs, 1, 1)
                    pr_prev_norm = tf.cast(tf.norm(pr_prev, axis=1, keepdims=True) ** 2, dtype=tf.float64)
                self.lr[t + 1] = tf.where(condition=mask,
                                          x=tf.math.real(tf.matmul(tf.linalg.adjoint(r), pr_prev))
                                            / pr_prev_norm,
                                          y=self.lr[t])
            ...
            if self.vec_step_size:
                ...
                if self.r_tune:
                    step_size = tf.where(condition=mask,
                                         x=tf.maximum(tf.math.sqrt(r_norm) / self.nt * tf.maximum(self.gamma[t + 1],
                                                                                       0), dqam_for_step) * alpha,
                                         y=step_size)
                else:
                    step_size = tf.where(condition=mask,
