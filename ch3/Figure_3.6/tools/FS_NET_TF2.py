...
class FSNet(Model):
    def __init__(self, trainset=None):
        ...
        # trainable variables
        self.weight, self.bia, self.t = [], [], []
        for t in range(self.iter):
            # 0.1x for QPSK;
            self.weight.append(tf.Variable(tf.random.normal([4 * self.nt, 1], dtype=tf.float64,
                                                            stddev=0.7),
                                           name='weight_' + str(t), trainable=True))  # note: the stddev is important
            self.bia.append(tf.Variable(tf.zeros([4 * self.nt, 1], dtype=tf.float64),
                                        name='bias_' + str(t), trainable=True))
            self.t.append(tf.Variable(0.1, dtype=tf.float64, trainable=True, name='t_' + str(t)))
        self.constellation_norm = tf.convert_to_tensor(self.constellation_norm)

    def __call__(self, x, y, A, noise_var, bs=None, test=False):  # todo: incorporate noise variance to improve
        ...
        for t in range(self.iter):
            ...
            # xhat = self.piecewise_linear_soft_sign(tmp[:, :2 * self.nt, :]) +\
            #        self.piecewise_linear_soft_sign(tmp[:, 2 * self.nt:, :])
        ...
        if test:
            indices = tf.argmin(abs(x_hat * np.ones((1, 2 * self.nt, self.m)) - self.constellation_norm),
                                axis=2)  # (bs, 2 * nt)
