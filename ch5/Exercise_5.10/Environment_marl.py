...
class Environ:
    ...
    def Compute_Performance_Reward_Train(self, action_band, action_power):

        actions = action_band  # the channel_selection_part
        power_selection = action_power  # power selection
                ...
                for k in range(len(self.vehicles)):
                    for m in range(len(channel_selection[k, :])):
                        ...
                        if i == k and j == m or channel_selection[i, j] < 0:
                            continue
                        V2V_Interference[k, m, channel_selection[i, j]] += 10 ** ((power_selection[i, j] -self.V2V_channels_with_fastfading[i][self.vehicles[k].destinations[m]][channel_selection[i,j]] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
        self.V2V_Interference_all = 10 * np.log10(V2V_Interference)


    def act_for_training(self, band, power):

        band_temp = band.copy()
        power_temp = power.copy()
        V2I_Rate, V2V_Rate, reward_elements = self.Compute_Performance_Reward_Train(band_temp, power_temp)
    ...
    def act_for_testing(self, band, power):
